						...
						<xsl:for-each select="gmd:CI_Citation">
							...
							<xsl:for-each select="gmd:date">
								<xsl:for-each select="gmd:CI_Date">
									$mdItem['MDDateTime'] = trim('<xsl:value-of select="gmd:date"/>');              // (7) Reference Date Stamp

									<xsl:for-each select="gmd:dateType/gmd:CI_DateTypeCode">
										$mdItem['MDDateType'] = trim('<xsl:value-of select="@codeListValue"/>');        // (8) Reference Date Type
									</xsl:for-each>
								</xsl:for-each>
							</xsl:for-each>
