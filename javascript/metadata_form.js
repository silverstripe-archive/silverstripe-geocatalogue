 ...
 * ----------- */
	...
	$('#showCoordinates').click( function(){
		$('#coordinates').toggle('fast', function(){
			if($('#coordinates').is(':visible')){
				$('#showCoordinates').removeClass('expand').addClass('collapse').text("I prefer choosing from your options");
				
			} else {
				$('#showCoordinates').removeClass('collapse').addClass('expand').text("Enter a custom location and coordinates");
			}
		});
	});
	...
	function fillRegions(){
		var tx1 = $("#RegisterMetadataForm_MetadataEntryForm_Places option:selected").text();
		var tx2 = $("#RegisterMetadataForm_MetadataEntryForm_Regions option:selected").text();
		var tx3 = $("#RegisterMetadataForm_MetadataEntryForm_TAs option:selected").text();
		var tx4 = $("#RegisterMetadataForm_MetadataEntryForm_OffshoreIslands option:selected").text();
		var tx5 = $("#RegisterMetadataForm_MetadataEntryForm_Dependencies option:selected").text();
	...
	$("#RegisterMetadataForm_MetadataEntryForm_Regions").change(function () {
		...
		var regionName = $("#RegisterMetadataForm_MetadataEntryForm_Regions option:selected").text();
...
//		verifyRequired($("#RegisterMetadataForm_MetadataEntryForm_MDTopicCategory"));
		...
		if($('.validationError').size() > 0){
			...
			$('div.date select').removeAttr('disabled');
			return true;
		}
	});
  ...
  function removeFields(e) {
	...
	if ($(this).parent().attr('id').match('MDResourceFormatVersion')) {
		...
		$(this).parent().hide('fast', function() {
			...
			if ($(this).hasClass('date')){
				$('div.date:visible:last button').show();
				$('div.date:visible:last select').removeAttr('disabled');
	...
	$('a.addDate').click(function(e) {
		...
		if (!isDate($('div.date:visible:last input').val())) {
			...
			var usedDateTypes=$('div.date:visible select').map(function() {return this.value; });
			$('div.date:hidden:first select').children().each(function () {
				if (jQuery.inArray(this.value,usedDateTypes)!=-1) $(this).remove();
			});
  ...
  function cloneField(field) {
	...
	$('.MDHierarchyLevel select').live('click',function(e) {
		if ($(this).val()!="") {
